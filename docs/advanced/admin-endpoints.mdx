...
| `FLOW_ENABLED`            | Enable request-flow planning/runtime | `true`  |
| `FLOW_ADMIN_ENABLED`      | Expose flow plan/execution admin UI  | `true`  |
...
### GET /admin/api/v1/flows/plans

Returns the editable request-flow plan set currently active for the gateway. Plans can come from YAML, DB, or both depending on `FLOW_SOURCE`.

### PUT /admin/api/v1/flows/plans/{id}

Creates or updates a DB-backed request-flow plan. This endpoint is available only when `FLOW_ENABLED=true`, `FLOW_ADMIN_ENABLED=true`, and the flow source includes a writable DB layer (`db` or `hybrid`).

### DELETE /admin/api/v1/flows/plans/{id}

Deletes a DB-backed request-flow plan.

### GET /admin/api/v1/flows/executions

Returns recent per-request flow execution records, including the matched plan, applied guardrails, retry count, and whether failover was armed or used.
...
When request flows are enabled, the dashboard also exposes a `Flows` page for editing DB-backed plans and browsing recent flow execution history.
