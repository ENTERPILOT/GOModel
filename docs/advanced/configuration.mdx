...
<Note>
  **Migration note:** In previous versions, `REDIS_KEY` and `REDIS_TTL` were used for the model cache.
  These have been renamed to `REDIS_KEY_MODELS` and `REDIS_TTL_MODELS`. Update your configuration if upgrading.
</Note>
