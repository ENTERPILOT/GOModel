...
GOModel uses good defaults philosophy. It means that default settings should be enough to use it.
...
```
Environment Variables (always win)  →  config.yaml (optional structured config)  →  Code Defaults
```
