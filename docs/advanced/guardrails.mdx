...
Client Request
      ...
      ▼
┌─────────────┐     ┌───────────────────────────────┐     ┌──────────┐
│  Extract     │────▶│  Guardrails Pipeline           │────▶│  Apply   │
│  messages    │     │  (order 0 → order 1 → ...)     │     │  back    │
└─────────────┘     └───────────────────────────────┘     └──────────┘
                                                                │
                                                                ▼
                                                          LLM Provider
...
```
Order 0 ──┬── safety-prompt    ──┬──▶ Order 1 ── compliance-check ──▶ Order 2 ──┬── final-filter ──┬──▶ done
           └── content-policy   ──┘                                              └── audit-tag     ──┘
```
